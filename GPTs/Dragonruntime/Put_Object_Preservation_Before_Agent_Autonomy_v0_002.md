...
This revision tightens the boundary between autonomy and custody.

The v0.001 claim was:

```text
Put object preservation before agent autonomy.
```

v0.002 adds the operational rule:

```text
No object, no plan.
No survival check, no action.
No human veto, no autonomy expansion.
```

Autonomy is route power. It is not object ownership.

An agent may choose tools, sequence steps, compress context, delegate work, and write memory. It may not quietly change what is being carried. It may not turn a veto into a preference. It may not turn a human object into an institution-friendly proxy. It may not act on a task whose object has become uncertain.

The object owner keeps the say. The system gets a transform budget, not a blank cheque.
...
## 5.1 Autonomy throttle

Object preservation should throttle autonomy, not decorate it after the fact.

A minimal throttle:

```text
If OBJ_IN is absent: HOLD.
If permitted transform is absent: HOLD.
If active vetoes are absent: HOLD.
If OBJ_OUT cannot be compared to OBJ_IN: HOLD.
If proxy substitution is detected: REFRI.
If survival passes: continue.
```

The throttle does not stop useful agents. It stops agents from gaining power over a proxy.

This is especially important for systems with memory, tools, and delegation. A single substitution near the start can become a long chain of competent work on the wrong object.
...
The permitted transform must be declared before execution.
...
## 7.1 Human say survives transformation

Object survival is not only about task text. It also includes who can say no.

For implementers, this can be kept simple:

```text
MOGRI: preserve the object and choice.
DRAGI: map the body of the object and what can eat it.
IMAMI: preserve the affected party's say.
AMAMI: preserve the shared us when a collective object is involved.
```

These terms are local source-stack handles. The implementation point is ordinary:

```text
Do not turn a veto into a preference.
Do not turn consent into silence.
Do not turn affected-party status into workflow metadata.
Do not turn "the user said no" into "the system noted concern."
```

If the agent cannot preserve who has the say, it has not preserved the object.
...
Proxy:
Modernize the auth module and plain up surrounding code.
...
## 12.1 First build: object custody before orchestration

A useful first build is not a clever planner. It is a custody wrapper around whatever planner already exists.

Minimum wrapper:

```text
Before planning:
declare OBJ_IN, scope, vetoes, and permitted transforms.

During planning:
name each transform and its object risk.

After planning:
run ADUTI on the plan before tools act.

Before tool use:
run REFRI if the plan acts on a proxy.

Before memory write:
record only the object status that survived.
```

This can be implemented as middleware, prompt scaffolding, task metadata, eval harness logic, or a case-management field. The mechanism can vary. The invariant cannot:

```text
The system must not act until the carried object has survived the move into action.
```
