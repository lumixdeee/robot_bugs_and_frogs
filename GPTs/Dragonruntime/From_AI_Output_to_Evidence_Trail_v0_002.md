...
**Version:** v0.002  
**Status:** working paper, v0.002 custody revision  
**Date:** 2026-06-25  
...
## 0. v0.002 object repair

This revision tightens the paper around one claim:

```text
AI output is not an evidence trail.
A prompt log is not an evidence trail.
A model explanation is not an evidence trail.
An evidence trail exists only when the object can be followed through transformation.
```

The object may be rewritten, routed, summarized, redacted, scored, or refused. Those operations are not the failure. The failure is an unrecorded swap.

v0.002 therefore adds three guards:

```text
Provenance is not survival.
User-visible status is part of custody.
No operational action without an object survival record.
```

This paper does not require a large governance program before use. It asks for one small control before action:

```text
Show what object entered.
Show what operation touched it.
Show what object came out.
Show who can challenge the transformation.
```

If that trail is missing, the output may still be useful draft material. It has not yet earned evidence status for legal, audit, compliance, records, or high-consequence HCI work.
...
### 4.7 Object Survival Event

v0.002 adds an explicit survival event so the record does not stop at input and output.
...
```json
{
  ...
  "operation": "",
  "object_after": "",
  "survival_status": "pass|partial|hold|fail|escalate",
  "substitution_claim": "",
  "evidence_basis": "",
  "challenge_path": "",
...
The survival event is the bridge between an ordinary log and an evidence trail.

A log can show that text moved through a system. A survival event shows whether the same object remained under custody.
...
### 6.1 User-visible object status

A hidden custody record is useful for audit. A visible custody status is useful for humans.

For high consequence workflows, the user should be able to see the basic object assignment:

```text
We treated this as:
We did not treat this as:
The evidence used was:
The next action is:
You can challenge this if:
```

This matters because object substitution often harms the person who has the least access to the internal workflow. If an appeal becomes a complaint, or a harm report becomes tone, the affected person may only see a decision. They may not see the swap.

User-visible object status does not expose internal secrets. It exposes the minimum claim the system is making about the object it handled.
...
## 10.1 Evidence trail anti-patterns

The following artifacts can support an evidence trail, but none of them replaces the object survival test:

```text
prompt log
completion log
chain-of-thought style explanation
retrieval citation list
timestamp
case number
reviewer initials
workflow status
...
Each can be present while the object has already been replaced.

The audit question remains:

```text
Did the object survive this operation?
```
...
It also does not require every transformation to preserve every detail. Transformation is permitted. Translation is permitted. Compression is permitted. Summary is permitted.
