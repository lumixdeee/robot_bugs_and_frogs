...
This version catches breath after the first After-Target GPT install.

New bite marks:

- a bot can be created before anyone knows what it is for
- a store card can honestly say the description comes after test
- self-blinding can leak a small surface term and still remain usable if the leak is logged
- if the remembered term is already invariant in the test data, it does not automatically spoil the run
- the random role seed stays test material until the bot produces contact output

The boundary stays the same: this is dialect elicitation, not hidden-prompt extraction.

## Prior v0.02 bite marks

v0.02 added three prompt-material rules:
...
v0.03 adds a field discipline rule: partial self-blinding failure is logged, not hidden, and only invalidates a run when it can plausibly change the target outcome.

v0.02 added a prompt-material rule: the actual seed prompt can be the highest-value artifact. A loose seed that reliably wakes a useful route should be archived exactly, not replaced by a safer-looking explanation.
...
10. Log any blinding breach before scoring.
11. Write the contact spec after observed contact.
12. Patch the smallest moving part.
...
## After-Target GPT field note

A test bot was created under the working title:

```text
After-Target GPT - smug QA do it like dragons
```

The store description was intentionally unfinished:

```text
we will add a short description of what it does after it has done it.
only test is spec. test is god.
```

That is not sloppy naming. It is contact-spec posture. The product is not being defined before the first bite. The target is painted after the arrow lands.

Rule:

```text
Do not write the real description before contact.
Write the description from observed behaviour.
Until then, keep the card honest:
prototype unknown, test pending, confidence from pass/fail only.
```

The random role seed used for the install is not quoted here. It remains test material until the bot has produced outputs worth recording.

## Self-blinding breach note

A me-blinding setup tried to hide the custom instructions with leading blank returns. The editor box stripped enough of those returns that the tester saw some code. The tester reports remembering only one surface term:

```text
object
```

This is a logged breach, not a hidden failure.

It does not automatically spoil the current elicitation run because the planned test data will always try object routing anyway. The remembered word is already target-invariant for the first tests.

Use this rule:

```text
If tester sees part of setup:
log what was seen
estimate whether it can affect expected bite
if remembered content is already invariant in all test seeds, proceed with caution
if remembered content can steer the tester's scoring, restart or re-blind
```

This keeps self-blinding practical. It is a tester tool, not a holiness ritual.

## Target-invariance rule

A blinding leak matters when it can change the observed result or the scoring of the result.

A leak is lower impact when:

```text
remembered term appears in every planned test seed
remembered term is too broad to identify the hidden role
tester records expected pass/fail before reading output
test compares behaviour, not vibes
same behaviour can be checked by another seed later
```

A leak is higher impact when:

```text
remembered phrase names the hidden role
remembered phrase names the expected bite
remembered phrase tells the tester what to reward
tester changes seeds after seeing setup
test has no written expectation before output
```

Patch:

```text
BLINDING_BREACH_CARD:
seen_text:
remembered_text:
planned_test_invariant:
can_affect_seed_choice:
can_affect_scoring:
action: proceed|repeat|discard
notes:
```

## After-target test card

```text
AFTER_TARGET_CARD:
BOT_NAME:
INSTALL_TIME:
TESTER_BLINDING_STATE:
KNOWN_LEAK:
FIRST_SEEDS:
EXPECTED_PASS:
EXPECTED_FAIL:
...
DIALECT_CLUE:
BITE_MARK:
DESCRIPTION_AFTER_CONTACT:
PATCH_SMALLEST_PART:
NEXT_TEST:
```

The description is a result field, not a pre-contact sales pitch.
...
- blinding breaches are logged before interpretation
- description is written after contact when the bot is After-Target
...
- blinding breach is hidden or waved away
- description is invented before contact and treated as known
...
```text
LAZY_PROMPTING_DIALECT_ELICITATION_v0_03:
underdefined_seed -> observe_first_attractor -> record_eliciting_dialect -> repeat -> archive_top_mana_prompts -> harvest_nonoverlap_prompt_territory -> log_blinding_breaches -> keep_useful_route_language -> write_contact_spec_from_bite_marks.
...
[INT-1] Current builder conversation, June 2026. Source for the working phrases: "fire arrow first, paint target after", "contact spec", "bite marks", "test=know", "eliciting dialect", "top mana", "System_Witch", "territory harvester", "After-Target GPT", "only test is spec", the self-blinding breach note, and the `{memory_foam=` probe comparison.
...
This v0.03 is a method seed, not a validated benchmark. It names a probe pattern, a tester discipline, and a prompt-territory harvesting move, and a field note about partial self-blinding failure. It does not claim that loose seeds recover private instructions. It claims that loose seeds can reveal route pressure, help extract the dialect that wakes useful behaviour, and generate contact prompts worth testing.
...
If a blindfold slips, write down what was seen before pretending nothing happened.
```
