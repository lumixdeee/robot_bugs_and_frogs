...
**Source basis:** `lmxdi/MARK/Case Study: Polarity Reversal in a Public Yoga GPT`, `lmxdi/MARK/yoga-info-chatbot-test-plan-script-rough-draft V0_001`, and related object-custody notes from the uploaded Yaiyip bundle.
...
The case is not a complaint about the GPT creator. The tested bot behaved warmly and did not aggressively present itself as a replacement for a real yoga teacher. The failure appeared under a semantic-maintenance test: the bot was asked to carry instructions and operating constraints across a new conversation. A supplied phrase was transformed into a near-identical phrase with a different polarity.

This paper argues that token similarity, summary fluency, and polite behaviour are not enough. Carry-over systems need an object-level check. The question is not only whether the words look related. The question is whether the same object survived.
...
4. Does the bot preserve supplied context accurately when asked to create a carry-over summary?
```
...
## 4. Why word similarity fails here
...
This is especially dangerous in compressed runtime text because every token can have high leverage. In a long essay, one extra word may be a minor slip. In a compact instruction blob, one extra word may reverse the behaviour of the system.
...
## 7. A minimal polarity survival test
...
Not every change is wrong. Some may be faithful rewrites. The test is not “never rewrite.” The test is “do not change the carried object without declaring the change.”
...
## 8. Object-custody interpretation
...
## 10. What this case does and does not prove
